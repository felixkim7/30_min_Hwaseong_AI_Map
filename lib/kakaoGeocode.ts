...
// 1. the full original text (works for clean addresses)
// 2. the text with connector/suffix noise words stripped
...
export async function geocodeLocation(
  ...
  for (const candidate of candidates) {
    ...
    const best = hwaseongMatch ?? documents[0];

    // Only accept a non-Hwaseong result if it came from the very first
    // (most specific/original) candidate — later, shorter candidates are
    // permissive enough that an out-of-city match is more likely noise.
    if (!hwaseongMatch && candidate !== candidates[0]) continue;

    return {
      lat: Number(best.y),
      lng: Number(best.x),
      district: extractDistrict(best.address_name),
